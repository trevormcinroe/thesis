"""Really large, positive reward for picking up seems to work better?
		 - Fast increase performance in the beginning stages but is this compromised with unstable training later?


	upper-bound-actorlowlr
	upper-bound-2022
"""
...
"""
	actor_lr=1e-4 for upper-bound-actorlrlow
	upper-bound-2022 = (critic_target_updated = 8) -- bad divergence....
"""

replay_memory = ReplayMemory(mem_size=200000, s_shape=9 * 3, norm=False)
...
#
# 	if i % 10 == 0:
# 		print(f'Episode {i}, Pct: {np.mean(completed[-100:])}, R: {np.mean([reward_hist[-100:]])}, Hours time {(time.time() - start) / 3600}, a: {round(agent.alpha.item(), 4)}/{round(agent.log_alpha.item(), 4)}')
#
# 	if i % 1000 == 0:
# 		print('----------ACTION COUNTER-----------------')
# 		print(Counter(replay_memory.a.numpy()))
